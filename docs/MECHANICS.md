...
| Gearing (PROTOTYPE, off unless `CFG.gearsOn`, and TIME TRIALS ONLY) | Six ratios, 3.2x range, calibrated so gears 1 to 6 put the target at `ttCadence` across the TT's measured 5-16 m/s. **The gear MOVES the rhythm target** rather than adding a second cadence bar to disagree with the first: a bigger gear turns slower at the same road speed so it pulls the target down toward grinding, a smaller one pushes it up until you cannot tap fast enough. So there is no separate economy penalty and no spin-out cap — the wrong gear moves the target away from your taps, `hold` falls, and `hold` is already charged in both speed and legs. **The target is floored at `strokeTempo`** or the rule spirals: low speed makes a low target, a low target makes less power, and it chases itself to a standstill (measured: a 1799s ride against a normal 108s). A shift at full effort drops power ~15% for 0.34s and a shift while soft-pedalling costs nothing, so you shift before the drag. Gains 0.3-5.8s over sitting in one gear, growing with division, confirmed on two seed sets. **The road version was built, ridden and cut**: no fun, only difficulty, and its edge was inside the noise | `CFG.gearsOn`, `you.gear`, `you.cadTgt`, `you.gearIdeal`, `shiftBite` | `node tools/gearing.js` |
