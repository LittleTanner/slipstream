...
| AI drop-back | **Known gap, measured, open.** A rival's swing takes a median 11.91s against the code's own claim of four to six, and `swingCut` cannot fix it: every value above 0.12 breaks the wheel-suck escalation, because the mates overshoot the last wheel and never come back through. Needs a distance budget rather than a pace cut. See DECISIONS.md | `refusal.test.js` is the only gate that sees it |
