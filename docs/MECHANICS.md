...
| AI drop-back | A rival drops from the front to behind the last wheel in a median 3.65s, against the player's 3.3s. **The tail is the open problem**: the worst case runs to ~39s. An earlier claim of "median 11.91s" was a HARNESS FAULT (it timed contiguous runs of `swingOff`, which is re-armed by three paths) and is retracted. `swingCut` cannot fix the tail: every value above 0.12 breaks the wheel-suck escalation | `node tools/aiswing.js`; `refusal.test.js` is the only gate that sees a swing change |
...
| Gearing (PROTOTYPE, off unless `CFG.gearsOn`) | Six ratios, 3.2x range, 1.00 at gear 4. Cadence is road speed over the ratio, so the tap loop is untouched (taps are still effort) and the gear decides what cadence you would be turning at your current speed. Grinding and spinning out both fall out of that one line; past `cadMax` the power is CAPPED rather than taxed, so spun out is a real limit. The wrong gear costs ECONOMY, never speed. A shift at full effort drops power ~15% for 0.34s, a shift while soft-pedalling costs nothing, and that asymmetry is the whole reason to shift before the ramp. **Its competitive value is inside the noise** and the tuning does not reproduce between seed sets, so this is a question for the hand, not the harness | `CFG.gearsOn`, `you.gear`, `you.cad`, `shiftBite` | `node tools/gearing.js` (mechanism gates asserted, balance only reported) |
