...
| iOS conformance | `port/constants.json` (105 CFG values), `port/protocol.json` (timestep, input policy, sampling, tolerances) and `port/trace.json` (per-rider dist/speed/energy sampled once a second through a flat stage, a mountain stage and a time trial). **The golden says what a race RESULTS IN; the trace says what it DOES** - it turns "the answer is wrong" into "frame 14,220 is wrong". Proven to catch a 0.0001% typo in one constant within 1-9s of race time. Regenerate whenever the golden is regenerated | `node tools/port-export.js && node tools/port-verify.js`, in CI; see `docs/PORT.md` |
...
| AI drop-back | A relieved rider takes exactly the deficit that clears the gap it STILL has to drop in `swingSecs`, measured against the front's pace (or its own while it is still nominally on the front), floored at `swingFloor` of its own target. Median **3.08s**, worst 5.38s, against the player's 3.30s / 8.90s. **It only beat the flat-9% rule it replaced once it applied THROUGHOUT**: gated on `bf !== r` it switched itself off for the early swing and read 3.90s / 28.68s. A rule that stops applying halfway through is two rules, and the seam is where the tail was. `swingSecs` 4.0 / `swingFloor` 0.74 chosen for REALISM over score - 2.2/0.66 reads 2.01s and was rejected because a rival must not drop back quicker than you. TWO retracted figures came from the harness, not the game: "median 11.91s" timed contiguous runs of `swingOff`, and "worst 39.07s" started its clock on `breakFront` changing in breaks that were NOT ROTATING. `swingCut` above 0.12 still breaks the wheel-suck escalation | `node tools/aiswing.js`; `refusal.test.js` is the only mechanics gate that sees a swing change |
