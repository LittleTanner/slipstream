...
const Sim = require(process.env.SIM || '/home/user/slipstream/tools/sim.js');
const { CFG } = Sim;
...
for (let g = 0; g < 120 * 150; g++) {
  ...
  const t = g / 120;
  if (g % 600 === 0) console.log('t' + t.toFixed(0) + ' sinceTurn=' + (race.you.sinceTurn||0).toFixed(1)
    + ' front=' + (race.breakFront === race.you ? 'ME' : (race.breakFront ? race.breakFront.name : '-'))
    + ' called=' + race.turnCalled + ' rot=' + (race.rotating) + ' ahead=' + ahead.toFixed(1));
